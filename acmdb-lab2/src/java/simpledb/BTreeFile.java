...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        Iterator<BTreeEntry> iterator = ((BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY)).iterator();
        while (true) {
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        BTreeLeafPage brother = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
        dirtypages.put(page.getId(), page);
        dirtypages.put(parent.getId(), parent);
        dirtypages.put(brother.getId(), brother);
        ...
        for (int i = 0; i < moved; i++) {
            ...
            brother.insertTuple(tuple);
        }
        BTreeEntry splitting = new BTreeEntry(iterator.next().getField(keyField), page.getId(), brother.getId());
        parent.insertEntry(splitting);
        page.setParentId(parent.getId());
        brother.setParentId(parent.getId());
        brother.setLeftSiblingId(page.getId());
        brother.setRightSiblingId(page.getRightSiblingId());
        if (page.getRightSiblingId() != null) {
            BTreeLeafPage right = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
            right.setLeftSiblingId(brother.getId());
        }
        page.setRightSiblingId(brother.getId());
        return field.compare(Op.LESS_THAN_OR_EQ, splitting.getKey()) ? page : brother;
    }
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
            ...
            throws DbException, IOException, TransactionAbortedException {
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
        BTreeInternalPage brother = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
        dirtypages.put(page.getId(), page);
        dirtypages.put(parent.getId(), parent);
        dirtypages.put(brother.getId(), brother);

        int moved = (page.getNumEntries() - 1) / 2;
        Iterator<BTreeEntry> iterator = page.reverseIterator();
        for (int i = 0; i < moved; i++) {
            BTreeEntry entry = iterator.next();
            page.deleteKeyAndRightChild(entry);
            brother.insertEntry(entry);
        }
        BTreeEntry upEntry = iterator.next();
        BTreeEntry splitting = new BTreeEntry(upEntry.getKey(), page.getId(), brother.getId());
        page.deleteKeyAndRightChild(upEntry);
        parent.insertEntry(splitting);
        page.setParentId(parent.getId());
        brother.setParentId(parent.getId());
        updateParentPointers(tid, dirtypages, brother);
        return field.compare(Op.LESS_THAN_OR_EQ, splitting.getKey()) ? page : brother;
    }
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        for (int i = 0; i < move; i++) {
            cur = iterator.next();
            sibling.deleteTuple(cur);
            page.insertTuple(cur);
        }
        entry.setKey((isRightSibling ? cur : iterator.next()).getField(keyField));
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        dirtypages.put(parent.getId(), parent);
        BTreeEntry movedParentEntry = new BTreeEntry(parentEntry.getKey(), leftSibling.reverseIterator().next().getRightChild(), page.iterator().next().getLeftChild());
        int move = (leftSibling.getNumEntries() - page.getNumEntries()) / 2 - 1;
        page.insertEntry(movedParentEntry);
        ...
        BTreeEntry pushUp = iterator.next();
        parentEntry.setKey(pushUp.getKey());
        parent.updateEntry(parentEntry);
        leftSibling.deleteKeyAndRightChild(pushUp);
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        dirtypages.put(parent.getId(), parent);
        BTreeEntry movedParentEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), rightSibling.iterator().next().getLeftChild());
        int move = (rightSibling.getNumEntries() - page.getNumEntries()) / 2 - 1;
        page.insertEntry(movedParentEntry);
        ...
        parentEntry.setKey(pushUp.getKey());
        parent.updateEntry(parentEntry);
        rightSibling.deleteKeyAndLeftChild(pushUp);
        updateParentPointers(tid, dirtypages, page);
    }
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        dirtypages.put(parent.getId(), parent);
        for (Iterator<Tuple> iterator = rightPage.iterator(); iterator.hasNext(); ) {
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        for (Iterator<BTreeEntry> iterator = rightPage.iterator(); iterator.hasNext(); ) {
            BTreeEntry cur = iterator.next();
            rightPage.deleteKeyAndLeftChild(cur);
